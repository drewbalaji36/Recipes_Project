...
@require_GET
def safe_search_by_country(request):
    q = request.GET.get("q", "")
    with connection.cursor() as cur:
        cur.execute(
            "SELECT name, country FROM kitchen_recipe WHERE country = %s LIMIT 25",
            [q],
        )
        rows = cur.fetchall()
    data = [{"name": r[0], "country": r[1]} for r in rows]
    return JsonResponse(data, safe=False)
@transaction.atomic
def bulk_bump_spice_for_country(request):
    if request.method == "POST":
        country = request.POST.get("country")
        if country:
            qs = Recipe.objects.select_for_update().filter(country=country)
            for r in qs:
                r.spice_level = min(5, r.spice_level + 1)
                r.save()
            return redirect("recipes:list")
    countries = Recipe.objects.values_list("country", flat=True).distinct().order_by("country")
    return render(request, "kitchen/bulk_spice.html", {"countries": countries})
